...
#
# Refuses to signal a PID file whose process isn't the server we started
# (verified via the per-start --brainstorm-server-id argument visible in
# /proc/<pid>/cmdline or `ps`). A stale PID file pointing at an unrelated
# process after a reboot or PID wraparound is marked stale_pid and removed
# without signalling.
