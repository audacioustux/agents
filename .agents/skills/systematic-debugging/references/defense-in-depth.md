...
Leave that evidence in the tests, not in the commit history. Each layer you add needs a test that drives the bypass it closes and fails when that layer is removed — step 4 below. A layer with no such test is indistinguishable from paranoia once the session ends, and a later cleanup pass will delete it correctly.
